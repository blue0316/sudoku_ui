...
import { SudokuCtx } from "../../../App";

export default function ConnectWallet(props) {
  const wallet = useContext(SudokuCtx);
  ...
  useEffect(() => {
    wallet && wallet !== "" && setAddress(wallet);
  }, [wallet]);
  ...
  return (
    <button
      className="flex justify-center self-center gap-3 md:gap-4 text-[16px] md:text-[20px] w-[173px] h-[51px] md:w-[232px] md:h-[64px] btn-wallet"
      onClick={walletConnect}
    >
      <p className="self-center">{address ? address : "Connect Wallet"}</p>
